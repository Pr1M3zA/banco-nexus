...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const { res: resCuentas, alerta: alertaCuentas } = await fetchConAlerta("http://localhost:3001/api/cuentas");
        if (alertaCuentas) setAlertaLatencia(alertaCuentas);
        if (!resCuentas || !resCuentas.ok) throw new Error("Error al obtener cuentas");
        ...
        if (alertaCuenta) setAlertaLatencia(alertaCuenta);
        if (alertaHistorial) setAlertaLatencia(alertaHistorial);
